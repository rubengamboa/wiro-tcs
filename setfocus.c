...
/* JSW 29Sep2003 Added instructions for user. */
...
keypad( ) {
  ...
  sprintf(omsg, "Focus Control box must be switched to remote." );
  SLsmg_gotorc(5,0);
  SLsmg_write_string( omsg );

  sprintf(omsg, "Type 1 to move in, 3 to move out, & 2 to change speed." );
  SLsmg_gotorc(7,0);
  SLsmg_write_string( omsg );

  sprintf(omsg, "Type any other key to halt focus motor." );
  SLsmg_gotorc(8,0);
  SLsmg_write_string( omsg );

  sprintf(omsg, "Type any key to update focus position readout." );
  SLsmg_gotorc(9,0);
  SLsmg_write_string( omsg );
