/* track.c */
Code to track the WIRO 2.3m telescope under LynxOS          
...
#define DIAGPRIORITY	17	/* Set diagnostic thread priority */
#define SCREENPRIORITY	19	/* Set default screen priority */
#define TRACKPRIORITY	23	/* Set default tracking priority */
#define TRACKTIME 70000	/* Time in micro secs between track updates */
#define MAINPRIORITY  17  /* Priority of the parent task */
...
void read_hifi      (void);    /* Read hifi */    
void write_hifi     (void);    /* Read hifi */    
...
/* Dome X4 fix -- temporary */
/* int raw_dome_enc = 0;		Current raw dome encoder:  256 per 1/4 turn	*/
/* int last_raw_dome_enc= 0;	Previous raw dome encoder reading			*/
/* int dome_qturn   = 0;		Dome quarter turn:  0 to 3					*/
/* int sim_dome_enc = 0;		Simulated dome encoder:  256 per turn		*/
...
{       
	...
	tinfo = (struct wiro_memory *) 
		smem_get("WIRO_MEMORY",sizeof(tinfodata),SM_READ | SM_WRITE);
	if (tinfo == NULL) {
		printf("Unable to grab shared memory\n");
	}
	...
	if (drive_present | encoder_present | paddle_present )
		...
		gpibport_setup();       /* This opens up the appropriate channel */
	...
	/* Now detatch the threads that do the telescope and screen */
	/* First, create attribute structures */

	pthread_attr_create(&thatScreen);  /* Create the screen attributes */
	pthread_attr_create(&thatTrack);  /* Create the tracking attributes */
	pthread_attr_create(&thatStore); /* Create storestate attributes */
	pthread_attr_create(&thatDiagnostic); /* Create diagnostic attributes */
			
	pthread_attr_setinheritsched(&thatScreen, 0); /* Don't inherit from parent*/
	pthread_attr_setinheritsched(&thatTrack, 0);

	pthread_attr_setprio(&thatScreen,SCREENPRIORITY);  /* Set priorities */
	pthread_attr_setprio(&thatTrack,TRACKPRIORITY); 
	pthread_attr_setprio(&thatDiagnostic,DIAGPRIORITY); 

			/* Report the priorities */
	printf("Screen thread priority: %d\n", pthread_attr_getprio(&thatScreen));
	printf("Track thread priority: %d\n", pthread_attr_getprio(&thatTrack));
	printf("Store thread priority: %d\n", pthread_attr_getprio(&thatStore));
	fflush(stdout);
	
			/* Now, cut loose the threads */	
	pthread_create(&thTrack, thatTrack,trackloop,&totask);  /* Launch threads */
	...
	if (DiagnosticMode != NULL)
	  {
	  printf("Diagnostic thread priority: %d\n", 
	  		 pthread_attr_getprio(&thatDiagnostic));
	  pthread_create(&thDiagnostic, thatDiagnostic, do_diagnostic, &totask);
	  }
	else  printf("\n\nRosencrantz >");  /* Tell the user his shell's back */
	fflush(stdout);
	
	logf( "track" );
	pthread_join( &thTrack,  stat);	/* Wait for them to rejoin before quiting*/
	pthread_join( &thScreen, stat);
	pthread_join( &thStore, stat);
	pthread_join( &thDiagnostic, stat);
}
...
void do_screens(void ) {
	...
	new.it_value.tv_sec = (time_t) 1; /* setup delay times */
	new.it_value.tv_usec = (time_t) 0;
	new.it_interval.tv_sec = (time_t) 0; /* Do the screen every second */
	new.it_interval.tv_usec = (time_t) SCREENTIME ;

	mask[1] =0 ;   
	mask[0] =1 << (SIGALRM-1); 
		/* Set up a mask to look for the SIGALRM signal */

	if ( !nosigblock)
		sigblock(1L << (SIGALRM -1)); /* Mask the alarm signal */

	if ( 0 != setitimer(ITIMER_REAL,&new,&old)) {
		printf("Problem setting up timer %d error: \n",errno);    
	}

 	if ( 0 != getitimer(ITIMER_REAL,&new)) {
 		printf("Trouble reading getitimer: %d\n",errno);
 	} 
	...
	while(tinfo->keep_tracking != TRACK_STOP) {
		...
		tminn();  			 /* do the tracking screen */
		loopnumber++;
	 	sigwait(mask, &v);	 /* Wait for the next tick. */ 
	}
}
...
void trackloop(void)
{
	...
	new.it_value.tv_sec = (time_t) 1; /* setup delay times */
	new.it_value.tv_usec = (time_t) 0;
	new.it_interval.tv_sec = (time_t) 0;
	new.it_interval.tv_usec = (time_t) TRACKTIME ;
	...
	while(tinfo->keep_tracking != TRACK_STOP)  {
		...
		switch(DiagnosticMode)
		  ...
		  default:
			...
			/* To conserve time, only do one of the following */	
			/* In temporary modification for Ames, the hifi is always read */
/*			if 		( HIFI[0] == 1) read_hifi();
		    else if ( HIFI[0] == 2) write_hifi();
		    else    do_paddle();   */ /* read paddle mux and interpret */
...
{	struct timeb timeonobj;
	...
	case SOLAR :  
		...
		{  
			...
			DES_RA =  ORIG_RA + V_RA *(timesize(timeonobj)/3600.0);
			DES_DEC = ORIG_DEC+ V_DEC *(timesize(timeonobj)/3600.0);
...
void read_ut( )                     
{
	...
	ftime( &CURRENT );    /* Load up the current time counter */
	CURRENT.millitm = (int) tp.tv_usec / 1000.0; 
		/* Kludge because LynxOS doesn't appear to update millitm! */ 
