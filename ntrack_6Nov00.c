...
/* While adjusting the focus remotely in setfocus, alternate trackloop passes
between reading the focus and writing to the focus motor.  Read_hifi and
write_focus accomplish this by setting and clearing HIFI[0] bit 1. */

/* The eight bits in HIFI[0] contain no data from the HIFI D80; it is
reserved for flags.  HIFI[1] contains data from HIFI D80 channel 2 port 5.
The msb of HIFI[1] is the msb of all channel 2 (40 bits) and the msb of port
5 (8 bits).  The focus readout is contained in the twelve bits which are
contained in all of HIFI[3] and the least significant 4 bits of HIFI[2].
The fourth-least-significant bit of HIFI[2] is the msb of the focus readout.
The lsb of HIFI[3] is the lsb of the focus readout.  The four least significant
bits of HIFI[5] are wired as outputs to the dedicated Focus Control box which
is located in the Control Room Rack.  HIFI[5] contains the data from HIFI D80
Channel 2 port 1.  The fourth-least-significant bit of HIFI[5] toggles
Fast/Slow.  The 3rd-lsb commands In.  The 2nd-lsb commands Out.  The lsb of
HIFI[5] is currently an unused spare. */
